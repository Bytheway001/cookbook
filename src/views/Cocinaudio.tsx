...
export const Cocina = () => {
    ...
    return (
        <>
            <TopBar title="Interfaz Cocina" action="/" />
            <div className='d-flex flex-column justify-content-center content align-items-center'>
                {
                    connected ?
                        (<>
                            <p>Conectado {listening ? "Escuchando" : "B"}</p>
                            <p>{transcript}</p>
                        </>
                        )

                        :
                        <Button size='lg' variant='success' onClick={() => setConnected(true)}>Conectar</Button>
                }
            </div>
        </>
    )
